...
$stmt = $pdo->prepare("SELECT agent_id FROM agent_branches WHERE branch_id = ?");
$stmt->execute([$branchId]);
$assignedAgents = array_column($stmt->fetchAll(), 'agent_id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($siteId) || empty($branchCode)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE branches SET site_id = ?, branch_code = ?, balance = ? WHERE id = ?");
            $stmt->execute([$siteId, $branchCode, $balance, $branchId]);
            
            $stmt = $pdo->prepare("DELETE FROM agent_branches WHERE branch_id = ?");
            $stmt->execute([$branchId]);
                            ...
                            <div>
                                <label style="font-weight: normal;">
                                    <input type="checkbox" name="agents[]" value="<?php echo $agent['id']; ?>"
                                           <?php echo in_array($agent['id'], $assignedAgents) ? 'checked' : ''; ?>>
