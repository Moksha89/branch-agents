...
try {
    ...
    if ($transaction['deleted']) {
        echo json_encode(['success' => false, 'error' => 'Transaction already deleted']);
        exit;
    }
    
    $stmt = $pdo->prepare("UPDATE transactions SET deleted = 1, modified = 0 WHERE id = ?");
    $stmt->execute([$transactionId]);
    ...
    echo json_encode(['success' => true, 'message' => 'Transaction soft deleted successfully']);
